...
) : LWLibavVideoSource{}
{
    ...
    if (opt->apply_repeat_flag == 1)
    {
        if (vohp->repeat_requested && !vohp->repeat_control)
            env->ThrowError("LWLibavVideoSource: repeat requested for %d frames by input video, but unable to obey (try repeat=0 to get a VFR clip).", vohp->repeat_requested);
    }
